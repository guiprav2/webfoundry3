...
function insertBeforeWithAnchoredNodes(parentEl, n, n2, c) {
  if (Array.isArray(n)) {
    n.forEach(function(n) { c = insertBeforeWithAnchoredNodes(parentEl, n, n2, c) });
    return;
  }
  ...
  if (c !== n) {
    ...
    if (n.anchoredNodes) { c = insertBeforeWithAnchoredNodes(parentEl, n.anchoredNodes, n2, c) }
  } else {
    c = c?.nextSibling;
  }
...
function mapAnchorBindingUpdate() {
  ...
  for (let x of self.valueMap.keys()) { !newArray.includes(x) && self.valueMap.delete(x) }
