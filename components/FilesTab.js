        ...
        ${d.map(() => this.state.app.files, ([name, path, isDir]) => d.html`
          <a href="#" ${{
            ...
            <div class="relative top-[-1px] flex gap-2">
              ${isDir && d.html`<button class="nf nf-fa-plus outline-none" ${{ onClick: () => this.post('app.newFile', path ? `${path}${name}` : name) }}></button>`}
              <button class="nf nf-fa-pencil outline-none" ${{ onClick: () => this.post('app.renameFile', path ? `${path}${name}` : name, isDir) }}></button>
              <button class="nf nf-fa-trash outline-none" ${{ onClick: () => this.post('app.deleteFile', path ? `${path}${name}` : name, isDir) }}></button>
            </div>
          </a>
        `)}
        <div class="flex-1"></div>
        <button ${{ class: ['border border-[#949ba4] rounded px-3 py-1'], disabled: () => this.sharing, onClick: () => this.post('app.shareUrl') }}>
          ${d.if(() => !this.sharing, 'Get share URL', 'Uploading...')}
        </button>
      </div>
    </div>
  `;
