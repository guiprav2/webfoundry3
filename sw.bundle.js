...
self.addEventListener('fetch', event => {
  ...
  } else if (event.request.url.includes('/preview/')) {
    const url = new URL(event.request.url);
    const path = decodeURIComponent(url.pathname);

    // Extract the file name and prepend with 'file:' to match the localForage key
    const parts = path.slice(path.indexOf('/preview/') + '/preview/'.length).split('/');
    const site = parts.shift();
    const fileKey = 'webfoundry:file:' + site + ':' + (url.pathname.endsWith('.html') ? 'index.html' : parts.join('/'));
...
async function handlePlaceholderRequest(query) {
  const unsplashApiKey = 'YOUR_UNSPLASH_API_KEY'; // Replace with your Unsplash API key
  const unsplashApiUrl = `https://api.unsplash.com/search/photos?query=${encodeURIComponent(query)}&client_id=fsxSItNPRTWBYpxCkwesJyID6QjthnGWtaQoZma4X-o`;
  ...
  try {
    ...
    return new Response('Error fetching data from Unsplash.', { status: 500 });
  }
}
